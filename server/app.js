...
app.post("/register", async (req, res) => {
    ...
    if (existingUserCount > 0) {
        return res
            .status(400)
            .json({
                message: "User already exists. Registration not allowed."
            });
    }
...
app.get("/books/:gradeLevel", async (req, res) => {
    ...
    try {
        ...
        res
            .status(500)
            .json({
                error: "Failed to retrieve books for this grade level"
            });
    }
});
...
app.post("/books", async (req, res) => {
    ...
    try {
        ...
        res
            .status(500)
            .json({
                error: "Failed to add/update book in the database"
            });
    }
});
...
app.post("/rental", async (req, res) => {
    ...
    try {
        ...
        res
            .status(500)
            .json({
                error: "Failed to save rental details to the database"
            });
    }
});
...
app.get("/rental-details", async (req, res) => {
    try {
        ...
        res.json({
            data: allRentalDetails
        });
    } catch (error) {
        res.status(500).json({
            error: "Failed to get all rental details"
        });
    }
});
...
app.get("/overdue-rentals", async (req, res) => {
    try {
        ...
        const overdueBooks = await Rental.find({
            dueDate: {
                $lt: today
            }
        });
        ...
        res.status(500).json({
            error: "Failed to get overdue books"
        });
    }
});
